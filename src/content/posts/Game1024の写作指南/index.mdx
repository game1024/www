...
import 搜索引擎架构 from "./搜索引擎架构.webp"
import Transformer from "./Transformer.webp"
import TransformerArch from "./TransformerArch.webp"
...
也可以使用`ins=`和`del=`来高亮指定行，分别使用不同的颜色来表示新增和删除的代码行
::::tabs
  :::tab{label="效果预览"}
...
## 专注模式
当添加`focus`属性后，代码块将进入专注模式，未高亮的行会降低透明度
...
## 标记关键词
可以在代码块的语法中使用`mark=/regex/flags`来标记关键词
::::tabs
  :::tab{label="效果预览"}
...
::::::tabs
  :::::tab{label="效果预览"}
    ::::chat{model="gemini"}
        :::user
        <ChatContext>
          <Image src={TransformerArch} alt="TransformerArch" width="1000"/>
        </ChatContext>
        帮我把上面的图片转化成黑板粉笔风格的手绘图
        :::
