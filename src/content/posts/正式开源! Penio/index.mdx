...
<video controls autoPlay loop className="block mb-4">
...
我们也可以在tauri.conf.json中设置`transparent=true`
```json title="tauri.conf.json" {3,7} focus
{
  "app": {
...
这是因为在x11系统上，需要在程序开头调用`XInitThreads()`来获得X11的线程支持，否则就会崩溃

解决方法
:::steps
1. 首先在`Cargo.toml`中添加x11包
   ```toml title="Cargo.toml"
   [target.'cfg(target_os = "linux")'.dependencies]
   x11 = "2.21.0"
   ```

2. 然后在`main.rs`中调用`XInitThreads()`
   ```rust title="main.rs" {2-8} focus
   fn main() {
       #[cfg(target_os = "linux")]
       {
           use x11;
...
```xml title="Package.appxmanifest" {3,6-10} focus
<Package 
  ...
...
```xml title="Package.appxmanifest" del={7-12} focus
<Package 
  ...
>
